...
Let's parse year and month using lubridate::(month) & lubridate::(year):
```{r}
lobster_parse_date <- lobster_date %>% 
  mutate(
    obs_month = lubridate::month(date_new, label = TRUE), obs_year = lubridate::year(date_new)
  )
  
```

Now let's find counds of obersved lobsters based on different hierarchial groupings:
...
There is a significance difference in lobster lengths between Naples and Mohawk Reef. (t(df) = statistic, p < 0.001, alpha = .05). 
